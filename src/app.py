...
# App configuration (always runs)
st.set_page_config(
    page_title="GHATZ Data Visualization",
    page_icon=":bar_chart:",
    layout="wide",
    initial_sidebar_state="expanded",  # 👈 Collapsible sidebar
    menu_items={
...
def check_password(username, password):
    if username not in st.secrets["usernames"]:
        return False
...
def main_app():
    ...
    with st.sidebar:
        ...
        st.session_state.option = st.selectbox(
            ...
            [
                "Home", "GHATZ Area Map", "Relief Well", "GHATZ Facilities", 
    ...
    if st.session_state.option == "Home":
        show_home_page()
    elif st.session_state.option == "GHATZ Area Map":
        show_area_map_page(client)
    elif st.session_state.option == "GHATZ Facilities":
        show_facilities_page(client)
    elif st.session_state.option == "Relief Well":
        show_ghatzReliefWell_page(client)
    elif st.session_state.option == "GHATZ Building Structures":
        ...
        show_ghatzWeather_page(client)
    elif st.session_state.option == "GHATZ Security":
        show_ghatzSecurity_page(client)
    elif st.session_state.option == "GHATZ Water Level":
        show_ghatzWaterLevel_page(client)
    elif st.session_state.option == "GHATZ Staff Composition":
        show_ghatzStaff_page(client)
    elif st.session_state.option == "GHATZ Standpipe":
        show_ghatzStandpipe_page(client)
    elif st.session_state.option == "GHATZ Vibrating Wire":
        show_ghatzVibratingWire_page(client)
    elif st.session_state.option == "GHATZ Seepage":
        show_ghatzSeepage_page(client)
...
def show_area_map_page(client):
                ...
                """)
def show_facilities_page(client):
                ...
                with st.expander("ℹ️ Condition Status Definitions"):
                    st.markdown("""
                        ...
                        """)
def show_ghatzBuilding_page(client):
                @st.cache_data(ttl=3600)
                ...
                if __name__ == "__main__":
                    main()
def show_ghatzReliefWell_page(client):
    ...
    # Main app flow
def show_ghatzWeather_page(client):
...
def show_ghatsAirValves_page(client):
...
def show_ghatzMachineries_page(client):
                ...
                # Call the function to run the app
                machinery_analysis()
def show_ghatzSecurity_page(client):
...
def show_ghatzWaterLevel_page(client):
...
def show_ghatzStaff_page(client):
