...
st.markdown('<style>div.block-container{padding-top:1rem; padding-bottom:0.0rem;}</style>', unsafe_allow_html=True)
st.markdown("""
<style>
...
if option == "Home":
    ...
    try:
        ...
        col1.metric("First Measurement", filtered_df['Date'].min().strftime('%Y-%m-%d'))
        col2.metric("Last Measurement", filtered_df['Date'].max().strftime('%Y-%m-%d'))
        col3.metric("Valid Measurements", len(filtered_df))
        
        st.divider()
        
        # Visualization tabs
        tab1, tab2, tab3 = st.tabs(["Time Series", "Depth Analysis", "Data Table"])
        ...
        with tab1:
            ...
            with col2:
                ...
                st.line_chart(filtered_df.set_index('Date')['Elevation Change'])
        
        with tab2:
            st.subheader("Depth Analysis")
            fig, ax = plt.subplots(figsize=(10, 6))
            ax.hist(filtered_df['Depth'], bins=15, edgecolor='black', color='teal')
        ...
        with tab3:
            ...
            st.dataframe(
                filtered_df.sort_values('Date', ascending=False)[
                    ['Date', 'RW_ID', 'Depth', 'Elevation', 'Depth Change', 'Elevation Change']
                ],
                column_config={
    ...
    try:
        ...
        st.line_chart(df.set_index("date")[["rightbank", "leftbank"]], use_container_width=True)
        
        # Monthly Aggregation
        df["month"] = df["date"].dt.to_period("M").dt.to_timestamp()
        monthly = df.groupby("month")[["rightbank", "leftbank"]].sum().reset_index()
    ...
    try:
        ...
        with col1:
            st.metric("Total Incidents", get_metric("INCIDENT SUMMARY", "Total Incidents"))
        with col2:
            st.metric("Fatalities", get_metric("INCIDENT SUMMARY", "Fatalities"))
        with col3:
            st.metric("Abductions", get_metric("INCIDENT SUMMARY", "Abductions"))
        ...
        with tab1:
            ...
            if not incident_types.empty:
                ...
                fig = px.pie(viz_df, names='Incident Type', values='Count', title='Incident Composition')
                st.plotly_chart(fig, use_container_width=True)
        ...
        with tab2:
            ...
            if not notable.empty:
                ...
                notable['Month'] = notable['Date'].dt.to_period('M').astype(str)
        ...
        with tab3:
            if 'Location' in security_data.columns:
                ...
                loc_counts = security_data[security_data['Location'].notna()]['Location'].value_counts()
    ...
    try:
        ...
        yearly_avg = df_filtered.groupby("year")["reservoir_level"].mean().reset_index()
        ...
        if not yearly_avg.empty:
            yearly_chart = alt.Chart(yearly_avg).mark_line(point=True).encode(
                ...
                y=alt.Y("reservoir_level:Q", title="Avg Reservoir Level", scale=alt.Scale(domain=[600, 630])),
                tooltip=["year", alt.Tooltip("reservoir_level:Q", title="Level", format=".2f")]
            ).properties(
                width="container",
                height=400,
        ...
        monthly_avg = df_filtered.groupby(["month", "month_num"])["reservoir_level"].mean().reset_index()
        monthly_avg = monthly_avg.sort_values("month_num")
        
        if not monthly_avg.empty:
            monthly_chart = alt.Chart(monthly_avg).mark_bar().encode(
                x=alt.X("month:N", sort=list(monthly_avg["month"]), title="Month"),
                y=alt.Y("reservoir_level:Q", title="Avg Reservoir Level", scale=alt.Scale(domain=[600, 630])),
                tooltip=["month", alt.Tooltip("reservoir_level:Q", title="Level", format=".2f")]
            ).properties(
                width="container",
                height=400,
    ...
    def load_staff_data():
        ...
        # Clean column names
        df.columns = df.columns.str.strip().str.lower()
        df.rename(columns={"names": "name", "position": "position", "location": "location"}, inplace=True)
    ...
    try:
        ...
        location_count = df["location"].value_counts().sort_values(ascending=True)
